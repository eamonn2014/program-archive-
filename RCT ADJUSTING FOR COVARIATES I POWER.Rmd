...
```{r prerequisites, echo=FALSE}
        ...
        path <- "Documents\\GIT\\Adjusting-for-covariates-I"  
...
for (j in 1:length(N)){
  ...
  for (i in 1: n.sims){
       ...
       effect.of.sex <- 5                # Hyp the effect of sex on outcome
       effect.of.treatment <- 1          # Hyp the treatment effect on outcome
...
```{r echo=FALSE, results='hide',   fig.cap="Effects of sex on outcome, simulated response outcomes top panels, mean response bottom", fig.show='asis'} 
      ...
      foo <- (post$b2)
       ...
       mtext(side=1, line=-1, text=paste(namez ,"effect adjusted to age", P.age,"; sex coded", P.sex,"; baseline", p4(P.baseline), sep=" "), adj=0, outer=T, cex=.7) 
